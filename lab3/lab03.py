...
class FA:
    ...
    def get_derivative(self):
        if self.type == '1':
            ...
            f_net = (1/2) * ((self.net / (1 + abs(self.net))) + 1)
            df_net = (1/2) / ((abs(self.net) + 1) ** 2)
            return df_net


def realF(vec_weights, vec_sets, vec_centers, func: FA):
    vec_y = list()
...
class RBF_neyr:
    def __init__(self, count_centers: int, vec_centers: list, func: FA, bias=1.):
    ...
    def correct_weights(self, learn_rate: float, delta: float, vec_fi: list):
        for j in range(len(self.centers) + 1):
            self.weights[j] += learn_rate * delta * vec_fi[j] * self.function.get_derivative()

    def reset(self):
        self.weights = [0 for _ in range(len(self.weights))]


def learn(vec_func, vec_centers, vec_sets, func: FA, learn_rate):
    vec_error = list()
    neuron = RBF_neyr(len(vec_centers), vec_centers, func)
    for number_sets in range(2, len(vec_sets)):
        for setX, setF in zip(itertools.combinations(vec_sets, number_sets),
            ...
            vec_y = realF(neuron.weights, setX, vec_centers, func)
            error = hemming(setF, vec_y)
            ...
            while error != 0 and len(vec_error) < 100:
                for set_i in range(len(setX)):
                    ...
                    neuron.correct_weights(learn_rate, delta, vec_fi)
                vec_y = realF(neuron.weights, setX, vec_centers, func)
                error = hemming(setF, vec_y)
                table.add_row([len(vec_error), copy.copy(neuron.weights), copy.copy(vec_y), error])
                vec_error.append(error)
            vec_y = realF(neuron.weights, vec_sets, vec_centers, func)
            if hemming(vec_func, vec_y) == 0:
...
if __name__ == "__main__":
    number_variables = 4
    learn_rate = float(input('Введите норму обучения:'))
    activation_func = FA(input('Введите ФА (1 = пороговая ФА, 2 = логическая ФА):'))
    ...
    weights, vec_errors = learn(target_function, centers_rbf_neurons, sets_x, activation_func, learn_rate)

    real_function = realF(weights, sets_x, centers_rbf_neurons, activation_func)
    print('Целевая функция', target_function)
    print('Реальная функция', real_function)

    plot([i for i in range(len(vec_errors))], vec_errors, "Error E", "Era K", "E(k)")
